...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const quiz = await prisma.quiz.findFirst({
    ...
    include: {
      tryouts: {
        include: { attempts: { where: { submittedAt: { not: null } }, orderBy: { submittedAt: "desc" } } },
      },
    },
  });
  if (!quiz) {
  ...
  for (const tryout of quiz.tryouts) {
  ...
  return new NextResponse(csv, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${quiz.id}-scores.csv"`,
    },
  });
}
